...
public ActionResult DeleteConfirmed(int id)
        {
            ...
            foreach (var account in AllAccounts)
            {
                ...
                {
                    var AccountTrans = db.SavingAccountTxnHistories.Where(x => x.AccNum == customerAccount.AccNum).ToList();
                    foreach (var trans in AccountTrans)
                        db.SavingAccountTxnHistories.Remove(trans); SavingAccountDetail savingAccount = db.SavingAccountDetails.Find(customerAccount.AccNum);
                    db.SavingAccountDetails.Remove(savingAccount);
                }
                else
                {
                    var AccountTrans = db.LoanEMIDetails.Where(x => x.AccNum == customerAccount.AccNum).ToList();
                    foreach (var trans in AccountTrans)
                        db.LoanEMIDetails.Remove(trans); LoanAccountDetail loanAccount = db.LoanAccountDetails.Find(customerAccount.AccNum);
            ...
            CustomerDetail customerDetail = db.CustomerDetails.Find(id);
            db.CustomerDetails.Remove(customerDetail); db.SaveChanges();
            return RedirectToAction("Search");
        }
